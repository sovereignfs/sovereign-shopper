...
- **DS-first: this plugin is a consumer.** Three primitives Phase 1 needs
  don't exist in `@sovereignfs/ui` yet — build them there, not plugin-local:
  1. **Checkable list row** (checkbox + icon + label + trailing stepper,
     whole-row tap target, strikethrough-on-checked).
  2. **Type-ahead / suggestion input** (text input + anchored result list).
  3. **Quantity stepper** (numeric input with +/− and a unit suffix).
...
- **Icon set is an open question** (SPEC.md open question 4 / UI.md open
  question 1) — `@sovereignfs/ui`'s `Icon.tsx` has ~26 generic icons, no
  grocery categories. Resolve before building `IconPicker.tsx` (roadmap T-06).
...
1. **Icon set** — sourcing/licensing for grocery-category icons, blocks T-06.
2. **Quantity stepper unit list** — fixed enum vs. free text with
   autocomplete, confirm before building.
3. **New-component ownership** — the three new `@sovereignfs/ui` primitives
   likely land as part of T-05/T-06 in this plugin's own roadmap PRs rather
   than a separate platform-side prerequisite, but confirm with the platform
   maintainer since it touches the shared design system.
